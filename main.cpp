...
void testJumble() {
	...
	//attempt to make a word that is too big ADDED TESTING FUNCTION
	try {
		cout << "\nAttempting to make a word that is too big, should display an error message" << endl;
		JumblePuzzle jp5("areallyreallyreallyreallylongword", "easy");
	}
	catch (BadJumbleException e)
	{
		cout << e.what() << endl;
	...
	//attempt to make a word that is too small ADDED TESTING FUNCTION
	try {
		cout << "\nAttempting to make a word that is too small, should display an error message" << endl;
		JumblePuzzle jp5("hi", "easy");
	}
	catch (BadJumbleException e)
	{
		cout << e.what() << endl;
	...
	//Test difficulty settings, along with getters
	cout << "\nTesting difficulty getters, should display word location and orientation for 3 puzzles" << endl;
	cout << "Sizes should be (8, 12, 16)" << endl;

	//easy mode
	JumblePuzzle ez("TEST", "easy");
	cout << "\nEASY PUZZLE" << endl;
	showJumble(ez.getJumble(), ez.getSize());
	cout << "Row: " << ez.getRowPos() << ", Col: " << ez.getColPos() << ", Direction: " << ez.getDirection() << ", Size: " << ez.getSize() << endl;
	
	//medium mode
	JumblePuzzle med("TEST", "medium");
	cout << "\nMEDIUM PUZZLE" << endl;
	showJumble(med.getJumble(), med.getSize());
	cout << "Row: " << med.getRowPos() << ", Col: " << med.getColPos() << ", Direction: " << med.getDirection() << ", Size: " << med.getSize() << endl;

	//hard mode
	JumblePuzzle hrd("TEST", "hard");
	cout << "\nHARD PUZZLE" << endl;
	showJumble(hrd.getJumble(), hrd.getSize());
	cout << "Row: " << hrd.getRowPos() << ", Col: " << hrd.getColPos() << ", Direction: " << hrd.getDirection() << ", Size: " << hrd.getSize() << endl;
